...
import Router, { useRouter }                from 'next/router'; // 경로이동
import { useEffect, userEffect,  useState   }  from 'react';       // 이벤트변경감지, 변수
...
function AppLayout({  children , initialUser  }){   //★ 대체부품, 초기값
    ...
    const handleLogout = () => { // 디스패치(전역 - logoutRequest()) 호출  / 경로 login 페이지로 넘기기
    ...
    const menuItems = [
        ...
        ?[
            ...
            { key: "logout",  label: <a onClick={handleLogout} style={{cursor:"pointer"}}>🔓 LOGOUT</a> },
        ]
        : [
