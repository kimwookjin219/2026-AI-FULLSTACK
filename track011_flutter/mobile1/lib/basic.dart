...
void main() {
  ...
  print("sum(10,3) = ${sum(3,5)}");
 
  // 7. 클래스(Class) 객체 생성 및 메서드 호출
  Person p = Person("홍길동", 20);
  p.sayHello();
 
  // 8. 비동기 처리 (Future: 미래에 완료될 작업, async/await: 작업동안 기다리기 , then: 작업완료 후)
  fetchData().then( (value) => print("then (1): $value") ); //#1 fetchData() 2초 타이머  / #2 then 예약
  testAsync(); //#3 fetchData() 다른 2초 타이머
}
// 비동기 함수 정의 (2초 후 완료)
Future<String> fetchData() async{
  await Future.delayed(Duration(seconds: 2));
  return "서버에서 데터 가져오기 완료!";
}
