...
# 1) flutter_riverpod : redux 처럼 전역상태 관리 라이브러리
# 2) dio : Axios 처럼 비동기 hhtp 통신 라이브러리 ( Intercepter 기능 제공 )
# 3) flutter_secure_storage : 브라우저의 localStorage/Cookies 대신 모바일 암호화 영역 JWT 저장
...
// ConsumerWidget 상속받으면 React에서 useSelector 처럼 전역 인증
...
```dart
// React [Redux + Saga + api/axios.js(JWT 토큰 처리)]
// AuthState - Redux
// _dio.interceptors : api 요청마다 `Bearer ${token}`을 헤더 주입, 401 에러토큰

import 'package:flutter_riverpod/flutter_riverpod.dart'; // Redux
...
class AuthNotifier extends Notifier<AuthState> {
  @override
  AuthState build() {
    _initDio(); // Provider 생성시 interceptor 설정 초기화
  ...
  Future<bool> _refreshAccessToken() async {
    try {
      ...
      await logout(); // refresh 실패시 강제 로그아웃
    }
    return false;
  }
  // 로그인
  ...
  Future<void> logout() async {
    ...
    await _storage.delete(key: 'accessToken'); // 저장된 토큰 삭제
    state = const AuthState(); // 상태 초기화
  }
  // 회원가입
  ...
  Future<bool> checkNicknameDuplicate(String nickname) async {
    try {
      ...
      return response.data; // true : 이미 존재 , false : 사용가능
    } catch (e) {
      return false;
    }
  }
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return AppLayout(
      child: Scaffold(
        ...
        // 로그인 된 상태만, 화면 우측 하단 둥근 플로팅 글쓰기 버튼 (FAB)
