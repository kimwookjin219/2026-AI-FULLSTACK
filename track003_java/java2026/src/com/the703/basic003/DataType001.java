...
public class DataType001 {
	public static void main(String[] args) {
		...
		fl = lo; //실수 > 정수 / fl(4byte) = l(8byte)지만 실수가 정수보다 크므로 가능!
		
		float fp1 = 1.0000001f; // 0 6개 + 1 =7자리  → 정확하게 표현
		float fp2 = 1.00000001f; // 0 7개 + 1 =8자리 → 정밀도 초과 반올림
		float fp3 = 1.11111111f; //                → 근사값 
		System.out.println(fp1 + "\t" + fp2 + "\t" + fp3);
		//                1.0000001	    1.0	       1.1111112
		
		double db1 = 1.000000000000001;  //0 14개 + 1 = 15개 → 정확하게 표현
		double db2 = 1.0000000000000001;  //0 15개 + 1 = 16개 → 정밀도 초과 반올림
		double db3 = 1.1111111111111111;  //0 15개 + 1 = 16개 → 근사값
		...
		System.out.println(abc == abc2); // 주소값이 달라서 false
	}
}
