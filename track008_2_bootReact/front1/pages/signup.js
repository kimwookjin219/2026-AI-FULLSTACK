...
//store : useSelector(전역), useDispatch(스토어의 이벤트 알림) 
// 감지 : useEffect(이벤트 변경감지), useState(변수 세팅) 
// 경로 : useRouter
...
//2. function (부품)
function SignupPage(){
    /// 5개 부품 - useEffect(이벤트 변경감지), useState(변수 세팅) 
    ...
    //////////////// Layout > Row > Col    Col
    // 모바일 제일 작은사이즈 : 24 xs={} / 모바일2 : 16 sm={} / 태블릿 : 8 md={} / lg={}
    return (<Row justify="center">
        ...
        <Form layout="horizontal" onFinish={onFinish}>
            {/* 이메일입력 + 이메일 중복검사 Form.Item   >   Input */}
            <Form.Item
                    label="이메일"
                    name="email"
                    hasFeedback
