...
select * from userinfo_select where 1=2; -- 1=2 ? 해당하는 값 X , 구조만 복사
...
update userinfo_select set age=null where name='first' and no=1;
...
update userinfo_select set no=10 where no=4 and age=44;
...
where age is not null and no in(2,3) and name not like '_e%';
...
select dept, job , count(*) `사원수`, max(sal) `최고급여`, sum(sal) `급여 합`, avg(sal) `평균급여`
from emp
group by dept,job
order by dept,job;
...
desc emp;
alter table emp modify dept int not null;
...
select 1.2345 
, round(12.2545, 1)  -- 소수점 1번째 자리까지 반올림
, round(15.2345, -1) -- 정수 1번째 자리까지 반올림
from dual;
...
-- mysql>
-- mysql>
-- mysql> -- 3. 찾기1 - 위치  instr  (문자열, 찾을 문자)
-- +----------+-----------+-----------+
-- | b의 위치 | ab의 위치 | ac의 위치 |
-- +----------+-----------+-----------+
-- |        2 |         1 |         0 |
-- +----------+-----------+-----------+
-- 1 row in set (0.00 sec)
select instr('abc','b') `b의 위치`, instr('abc','ab') `ab의 위치` , instr('abc', 'ac') `ac의 위치` -- mysql은 카운트 1부터 시작
from dual;
...
-- +------+------+------+
-- 1 row in set (0.00 sec)
select substr('abcde',2,2) `s1`, substr('abcde',2,3) `s2`, substr('abcde',1,3) `s3`
from dual;
...
-- mysql>    trim : 양쪽 공백 / ltrim : 왼쪽 공백 / rtrim : 오른쪽 공백
-- +---------+----------+----------+
-- | t1      | t2       | t3       |
-- +---------+----------+----------+
-- | #a b c# | #a b c # | # a b c# |
-- +---------+----------+----------+
-- 1 row in set (0.00 sec)
select concat('#',trim(' a b c '),'#') `t1`,concat('#',ltrim(' a b c '),'#') `t2`,concat('#',rtrim(' a b c '),'#') `t3`
from dual;
...
insert into fn_select_userinfo2 values(3,'ccc',33,'aaa@gmail.com');
insert into fn_select_userinfo2 values(4,'ddd',44,'ccc@gmail.com');
...
update select_userinfo2 set name='aaaa' where no=1 and name='aaa';
update select_userinfo2 set name='bbaab' where no=2 and name='bbb';
update select_userinfo2 set name='ccaac' where no=3 and name='ccc';
update select_userinfo2 set name='daadd' where no=4 and name='ddd';
update select_userinfo2 set name='baaca' where no=6 and name='bca';
...
-- |     3     |
-- +-----------+
select count(instr(name,'b') ) `b포함 인원`
from fn_select_userinfo2
