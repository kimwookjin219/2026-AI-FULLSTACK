...
// Set(주머니) - 순서 X , 증복 X / add,향상된 for/Iterator,size,remove,contains
...
public class Set001 {
	public static void main(String[] args) {
		...
		set1.add(1);  // 부품객체(클래스) = 기본값 (Integer - wrapper 클래스)   
		set1.add(1);  // 기본값을 자동으로 - 객체화 - 부품객체(wrapper 클래스)
		set1.add(2);
		set1.add(3);
		
		Integer i1 = 1; // 부품객체(클래스) = 기본값
		int i2 = i1;    // 기본값        = 부품객체
		float f = i1.floatValue();
		//float f2 = i2.floatValue(); //Cannot invoke floatValue() on the primitive type int
		...
		System.out.println(set2.remove( new Candy("츄파츕스" , 200))? "냠냠" : "못먹었어" );
