...
function* watchLogout(){
    ...
    // LOG_OUT_REQUEST 액션 발생 → 여러번 요청시 가장 마지막 요청을 처리 1개
...
function* watchLogin(){
    ...
    // LOG_IN_REQUEST 액션 발생 → 여러번 요청시 가장 마지막 요청을 처리 1개
    // return { ...state, isLoading: true , error:null };
}
...
function* watchSignUp(){
    ...
    // SIGN_UP_REQUEST 액션 발생 → 여러번 요청시 가장 마지막 요청을 처리 1개
    // return { ...state, isLoading: true , error:null };
}
...
export function loadUsersApi(data){
    return client.get( '/user' , data); // http://localhost:3065/user
}
...
function* watchLoadUsers(){
    ...
    // LOAD_USERS_REQUEST 액션 발생 → 여러번 요청시 가장 마지막 요청을 처리 1개
    // return { ...state, isLoading: true , error:null };
}
...
export function updateNicknameApi(data){
    return client.patch( `/user/${data.id}/nickname` , {nickname:data.nickname}); // http://localhost:3065/user/{id}/nickname
}
...
function* watchUpdateNickname(){
    ...
    // UPDATE_NICKNAME_REQUEST 액션 발생 → 여러번 요청시 가장 마지막 요청을 처리 1개
    // return { ...state, isLoading: true , error:null };
}
...
export function deleteUserApi(id){
    return client.delete( `/user/${id}` ); // http://localhost:3065/user/{id}
}
...
function* watchDeleteUser(){
    ...
    // DELETE_USER_REQUEST 액션 발생 → 여러번 요청시 가장 마지막 요청을 처리 1개
    // return { ...state, isLoading: true , error:null };
}
