...
```
1. float 속성을 사용할 때 부모 요소의 높이가 사라지는 문제를 해결하는 방법은?  overflow:hidden;
...
4. float와 display의 차이점을 간단히 설명하시오.     
    ...
    float: 왼쪽이나 오른쪽에 붙히고 끊는 속성(clear:both;,overflow-hidden)을 사용해야함 여백은 margin으로
    display: 박스들이 어떻게 놓일지 정하는 규칙(정리 도우미) 속성변환 가능
            flex는 자식요소들의 배치방법이 들어가는데 여백도 자동으로 맞추는게 가능함, 여백은 gap으로
...
6. margin: auto를 사용하여 블록 요소를 가운데 정렬하려면 어떤 조건이 필요한가?  
   display:flex; justify:center; flex-wrap: wrap;

7. z-index 속성은 어떤 경우에 사용되며, 값이 클수록 어떤 효과가 나타나는가?  

    position의 순위를 정할때 사용
    값이 클수록 위로 올라온다.
