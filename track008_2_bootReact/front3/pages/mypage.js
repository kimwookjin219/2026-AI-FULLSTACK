...
// 직접 API를 부르지 않고 , loadUserRequest 디스패치해서 saga 완료 기다림
export const getServerSideProps = wrapper.getServerSideProps((store) => async (ctx) => { 
  // 1. 요청헤더의 쿠키를 담아서 사용자 정보를 조회하기 위한 사가 액션  
  store.dispatch(loadUserRequest({ cookie: ctx.req.headers.cookie || "" }));
  // 2. 서버사이드에서 사가 작업이 끝날때까지 대기
  store.dispatch(END); // 채널 닫기 신호
  await store.sagaTask.toPromise(); 
  // 3. 스토어 상태를 확인해서 유저정보 없으면 로그인페이지로 리다이렉트
