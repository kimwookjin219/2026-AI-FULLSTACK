...
public class PostService {
	...
	//4. 게시글 생성 (save + 내용/해쉬태그 + 이미지 업로드)
	@Transactional // 레파지토리에 두번 접근해서 사용해야함
	public PostResponseDto createPost(Long userId, PostRequestDto dto, List<MultipartFile> files) {
		...
		if(dto.getHashtags() != null && !dto.getHashtags().isEmpty()) { 
			...
			Set<String> distinctTags = Arrays.stream(dto.getHashtags().split(",")) //1. , 기준으로 분리해서 배열을 스트림
                    ...
                    .filter(s -> !s.isEmpty())  // 3. 빈 공간 아닌 애들
			...
			distinctTags.forEach(tagStr -> {
                ...
                Hashtag tag = hashtagRepository.findByName(normalized) // 기존에 등록된 태그인지 확인
                        .orElseGet(() -> { // 존재하지 않으면
                            Hashtag newTag = new Hashtag(); // 새로운 Hashtag 만들고
                            newTag.setName(normalized); // 이름 셋팅
                            return hashtagRepository.save(newTag); // DB에 저장
                        });
	...
	//5. 게시글 수정 (save 안쓰고 update 쿼리 반영) #더티체크
	@Transactional
	public PostResponseDto updatePost(Long userId, Long postId, PostRequestDto dto, List<MultipartFile> files) {
		...
		post.setContent(dto.getContent()); // 저장 메서드를 따로 호출하지 않아도 update 쿼리 반영
		...
		return PostResponseDto.from( postRepository.save(post) ); // 더티체킹(Dirty Checking) 자동으로 update
	}
	...
	public void deletePost( Long userId , Long postId) {
		...
		post.setDeleted(true); // 저장 메서드를 따로 호출하지 않아도 update 쿼리 반영		
	}
