...
→ 클라이언트(브라우저가)가 HTML + JS를 이용해서 화면 그리기
...
├── test1.js                   # 모델 함수 테스트 스크립트
```
### 1. id : node / pass : react  oracl 유저만들기
```sql
--sqlplus
--conn  system/1234
--
...
patch: /user/{id}/nickname  ← rest 방식              데이터 접근방식 : url 자원소스가 포함
※비교  /user/nickname?id=1  ← queryString(쿼리스트링) 데이터 접근방식 : url?key=value
