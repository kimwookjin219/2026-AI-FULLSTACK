...
export function* signup(action){
    ...
    try{
        ...
        yield put(signupSuccess(result.data)); // 처리 결과 put
    }catch(err){
        yield put(signupFailure(err.response?.data?.message || err.message));
    }
}
//■1. takeLatest(signupRequest.type , signup) : takeLatest - 요청이 여러번 들어와도, 가장 마지막 요청만 처리
...
export function* fetchUser(action){
    //action = { type:user/fetchRequest , payload:1 }
    try{
