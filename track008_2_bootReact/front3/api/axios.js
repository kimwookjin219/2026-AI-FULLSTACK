...
const api = axios.create({
  ...
  // refreshToken이 HttpOnly 쿠키에 저장이 되어있으면 자동으로 포함 필요
...
// 요청 인터셉터 : 요청 보내기 전 Access Token을 헤더에 추가
api.interceptors.request.use(
  (config) => {
    if (typeof window !== "undefined") { // CSR 환경에서만 localStorage에 접근
  ...
  (error) => Promise.reject(error)  // 요청 에러처리
); 
...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (status === 401 && !original._retry) {
      original._retry = true; // 무한 루프방지 플래그
      try {
        ...
        original.headers.Authorization = `Bearer ${newAccessToken}`;   // 요청 헤더 갱신
        return api(original); 
      } catch (refreshErr) {
