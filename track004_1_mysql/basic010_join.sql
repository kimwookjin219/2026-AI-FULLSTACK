...
create table join_userban select * from userinfo where 1=2;
desc join_userban;
alter table join_userban modify no int not null auto_increment primary key;

alter table join_userban change age ban char(2);
...
-- +----+--------+-----+------+
-- >>> (1) = join
select a.no,a.name,a.age,b.ban   -- 안겹치면 소속 안밝혀도 됨. ex) a.age=age
from join_userinfo a, join_userban b
...
-- -- 이코드가 의미하는바는? emp,dept 테이블을 join해서 empno 오름차순으로 정렬
   -- 조인조건이 없으면 두 테이블 모두 조회
...
-- deptno 필드가 emp,dept 둘다 있어서 겹친다 / 겹치는 필드는 소속 밝히기
...
-- mysql>
SELECT EMPNO, ENAME, e.DEPT, DNAME, LOC
FROM EMP E, DEPT D
WHERE E.DEPT = D.DEPTNO;
...
select e.empno,e.ename,e.sal,e.dept,d.dname,d.loc
from emp e join dept d
on e.dept=d.deptno
where e.sal>=3000;
...
select s1.empno , s1.ename , s1.mgr , mgr.empno `mgr_empno` , mgr.ename `mgr_ename`
from emp s1, emp mgr , salgrade s
where s1.mgr=mgr.empno and s1.sal between s.losal and s.hisal 
...
select s1.empno, s1.ename , s1.mgr , s2.empno `mgr_empno`, s2.ename `mgr_ename`
from emp s1 , emp s2
...
select s1.empno, s1.ename , s1.mgr , s2.empno `mgr_empno`, s2.ename `mgr_ename`
from emp s1 left join emp s2
on s1.mgr=s2.empno
order by s1.empno asc;
...
desc dept;
desc emp;
alter table emp change dept deptno int not null;
...
-- +--------+------------+-----------+---------+---------+-----+
-- 3 rows in set (0.00 sec)
select d.deptno,dname,avg(e.sal) `avg_sal`,max(sal) `max_sal`,min(sal) `min_sal`,count(*) `cnt`
from emp e join dept d
on e.deptno=d.deptno
...
-- +--------+------------+-----------+---------+---------+-----+
-- 3 rows in set (0.00 sec)
select e.deptno,d.dname,avg(e.sal) `avg_sal`,max(e.sal) `max_sal`,min(e.sal) `min_sal`,count(*) `cnt`
from emp e join dept d
using(deptno)
...
on e.deptno=d.deptno
order by d.deptno,e.empno,d.dname asc;
...
-- mysql>
select d.DEPTNO ,d.DNAME, e.EMPNO ,e.ENAME , e.MGR , e.SAL  ,d.DEPTNO ,s.LOSAL, s.HISAL, s.GRADE, mgr.empno `MGR_EMPNO`, mgr.ename `MGR_ENAME` 
from dept d left join emp e 
on e.deptno=d.deptno -- 테이블끼리 데이터 연동
left join salgrade s
on e.sal between s.losal and s.hisal -- salgrade랑 연동 후 losal~hisal 사이
left join emp mgr
on e.mgr=mgr.empno -- 상사 찾기
...
select d.DEPTNO ,DNAME, e.EMPNO ,e.ENAME , e.MGR , SAL ,s.LOSAL, s.HISAL, s.GRADE, mgr.empno `MGR_EMPNO`, mgr.ename `MGR_ENAME` 
