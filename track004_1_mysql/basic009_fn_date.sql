...
-- 1. 시스템의 현재 시각 조회 
select now();          -- '2026-05-26 14:37:13'
...
select date_format('2026-05-26 14:37:13' , "%Y-%m-%d");
...
-- now() , weekday(0=월 ~ 6=일) , date_format(날짜, "%Y-%m-%d %H:%i:%s"), date_add, datediff ....
-- ■ 2. CRUD (SELECT Function-Date  연습문제)
...
alter table date_userinfo modify age int not null;
...
-- >> 연습문제2)  select_emp
-- now(), weekday, date_format, date_add m datediff
-- > 1. 금일날짜를 출력하시오.
select now();

-- > 2.  금일과  이번년도 크리스마스  날짜사이의 일수 차이를 구하시오
select datediff('2026-12-25',now()); -- 213
...
select date_add( now() , interval (6-weekday(now())) day ); -- 내일이 와도 가장 가까운 일요일

-- 지난 일요일
select date_add( now() , interval (weekday(now())-3)day);
