...
class Account {
    private int balance;    //인스턴스 변수 - this(각각) - heap area - new - 생성자          
    public static int accountCount=0;  // 클래스 변수 - 공유 - method area - new X 
    public final int id;    //인스턴스 변수 - this(각각) - heap area - new - 생성자           
...
public class Repeat002_oop {
    public static void main(String[] args) {
    	/* [method(정보,static,final)] : Account.class / Account.accountcount=0
...
Q1. 캡슐화(Encapsulation)란 무엇이며, 위 코드에서 어떻게 구현되었는지 설명하시오.
 ...
 1. 클래스는 부품객체이다
 2. 상태(멤버변수 : 클래스변수,인스턴스변수)와 행위(멤버함수 : 클래스메서드, 인스턴스 메서드)
 
 정의 : 외부에서 접근 제어
 구현 : balance를 private 선언, getBalance,deposit,withdraw 메서드를 통해서만 접근
...
Q3. static 키워드의 의미를 메모리 구조와 연결지어 설명하고, 위 코드에서 어떤 변수에 적용되었는지 쓰시오.
 ...
 의미 : 모든클래스가 공용으로 사용  
 메모리 : method area 저장 - 객체 생성(new)와 관계없이 접근
 사용방법 : 클래스명.변수명 / 클래스명.메서드명
     - Account.accountCount

Q4. final 키워드의 의미를 설명하고, 위 코드에서 어떤 변수에 적용되었는지 쓰시오.

 의미 : 변경불가 클래스(상속X) , 변수(상수) , 메서드(오버라이드 불가)
  
 id, 한번 초기화 되면 변경 불가, this.id = ++accountCount; 초기화

Q5. Account.accountCount의 값은 얼마인가? 왜 그렇게 되는지 설명하시오.

 클래스 변수
 값 : 2
 Account 객체가 2개 생성될때마다 기본 생성자가 호출되어서 ++accountCount 실행됨	
...
변수 : 상수(값 변경불가)
메서드 : 기능 수정 X,Override X
클래스 : 상속 X

Q10. 캡슐화의 장점은 무엇인가?

 - 데이터 보호(외부에서 직접 접근 차단)
 - 유지보수 용이함(내부 구현 변경 시 외부 영향 최소화)
