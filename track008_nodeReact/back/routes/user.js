 ...
 *   post:
 *     summary: 로그인 (테스트용)
 *     description: Passport 아메일과 비밀번호를 인증해서 세션저장
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
...
router.post('/logout', (req, res) => {   
    req.logout((err)=>{  // passport 제공함수 , 현재 로그인된 사용자 정보를 세션에서 제거
        ...
        req.session.destroy( ()=>{  // 세션을 제거
            res.clearCookie('connect.sid');   // 브라우저에 저장된 세션 쿠키를 삭제
...
router.get('/' , isAuthenticated ,async(req,res) => {
    try{
        ...
        res.status(500).json({message:'사용자조회 실패'}); 
    }
});

// patch: /user/{id}/nickname 닉네임 수정
/**
 * @swagger
 * /user/{id}/nickname:
 *   patch:
...
router.patch('/:id/nickname' , isAuthenticated , async(req,res) => {
    try{
        ...
        res.json({message:'닉네임수정 성공'});
    }catch(err){ 
        console.log('updateUserNickname' , err);
        res.json({message:'닉네임수정 실패'}); 
    }
});
