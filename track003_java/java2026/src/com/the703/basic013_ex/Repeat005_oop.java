...
class ZooKeeper1 {
    public void show(Animal1 animal) {
        ...
        if (animal instanceof Dog1) { ((Dog1) animal).playFetch(); } 
        else if (animal instanceof Cat1) { ((Cat1) animal).scratch(); }
        else if (animal instanceof Bird1) { ((Bird1) animal).fly(); }
    }
}
...
Q1.  다음코드를 확인하고 오류나는 부분을 체크하시오
 ...
 - 설계목적 , new X
 1) Animal ani = new Animal();   new 못함
 2) 상수 - 수정못함
 
    Animal.company = "(주) 네이버";
    System.out.println(Animal.company)
