...
class CleaningRobot extends Robot{
	...
	@Override void move() { System.out.println(super.model + "청소로봇: 청소중");  }
...
class SecurityRobot extends Robot{
	...
	@Override void move() { System.out.println(super.model + "경비로봇: 경비중");  }
...
class CookingRobot extends Robot{
	...
	@Override void move() {  System.out.println(super.model + "요리로봇: 요리중"); }
	@Override void speak() { System.out.println(super.model + "요리로봇: 오늘의 메뉴는 파스타입니다!"); }
}

public class AbstractEx001 {
	public static void main(String[] args) {
	      //Robot robot  = new Robot();  // Q1.why? 오류이유? abstract 클래스 - 구현부가 없음
	      System.out.println("\n--- 로봇 배열 시뮬레이션 ---");
	      //  부모              자식
	      Robot [] bots = { new CleaningRobot(), new SecurityRobot() , new CookingRobot() };
	      int[] levels = { 50, 70, 95 };
	      //bots[0] = 1번지 CleaningRobot { @charge() , @move() , @speak() }-Robot{ model,battery / charge() , move() , speak()}
	      //bots[1] = 2번지 SecurityRobot { @charge() , @move() , @speak() }-Robot{ model,battery / charge() , move() , speak()}
	      //bots[2] = 3번지 CookingRobot { @charge() , @move() , @speak() }-Robot{ model,battery / charge() , move() , speak()}
	      for(int i=0;i<bots.length;i++) { bots[i].model="Robo" + (i+1);  bots[i].battery = levels[i];  bots[i].charge(); bots[i].move(); bots[i].speak();}
	}
}
/*
연습문제1)
