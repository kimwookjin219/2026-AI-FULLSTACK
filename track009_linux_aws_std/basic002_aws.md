...
2. public ip
    > 52.78.236.160
3. ssh 클라이언트
    > ssh -i "moit.pem" ubuntu@ec2-52-78-236-160.ap-northeast-2.compute.amazonaws.com

    ※ moit.pem 보관주의    
    ```bash
    chmod 400 "moit.pem"  # 소유자 4: r-- , 그룹 0: --- , 다른 사람 0: ---
    ```
    - ctrl + `

    ```
    # 1. 상속 권한 완전히 제거
    icacls "moit.pem" /inheritance:r /grant:r "$($env:USERNAME):(R)"

    # 2. 혹시 남아있을 수 있는 다른 사용자 권한 강제 삭제
    icacls "moit.pem" /remove "NT AUTHORITY\Authenticated Users"
    icacls "moit.pem" /remove "BUILTIN\Users"
    icacls "moit.pem" /remove "NT AUTHORITY\SYSTEM"
    ``` 
■4. EC2에서 nginx
- 문지기
- 웹서버연결
...
- 애플리케이션을 docker 이미지로 빌드해서 ecr에 올려두면 어디서든지 가져다가 사용가능하게
    ...
    ※ ECR 검색
    1) 리포지토리 이름
    2) 이미지 태그 설정 - Mutable (연습용-latest 덮어쓸 수 있음)
    3) 암호화 설정 - 기본키 그대로
    ```
    284874840994.dkr.ecr.ap-northeast-2.amazonaws.com/the703
    ```
...
6. 필수 패키지 설정
    ...
    #   사용자계정  시스템그룹  docker  
    #   현재 로그인한 사용자에게 docker 그룹 권한을 줘서 sudo 없이 docker 명령어 사용 가능하게 
    ```
    ...
    8) 실행 디렉토리 생성
    ```bash
    #1. 상위 폴더 및 uploads 폴더까지 한번에 생성 (-p옵션)
    mkdir -p  /home/ubuntu/app/back/build/libs/uploads

    #2. 홈 프로젝트 기본 디렉토리 권한설정 (소유자는 모든권한, 그룹/타인은 읽기,실행권한)
...
- `AmazonECS_FullAccess`     →  ECS/Faragate 서비스관리 
    > (Github Actions, Jenkins 외부도구에서 빌드한 도커 이미지를 AWS ECR에 올리고)
    > 자동으로 배포해주는 권한 세팅
...
> https://github.com/kimwookjin219/track009_aws.git

2. Actions secrets and variables 시크릿키설정

```bash
Name: EC2_HOST      
Secret: 52.78.236.160
# 현재 ec2 퍼블릭 ip
...
Name: EC2_SSH_KEY
Secret:  
# .pem 파일의 내용을 그대로 붙혀넣기
 
Name: AWS_ACCESS_KEY_ID
Secret:  
# IAM에서 발급 받은 Access Key

Name: AWS_SECRET_ACCESS_KEY
Secret:  
# IAM에서 발급 받은 Secret Key
...
```
git clone https://github.com/kimwookjin219/track009_aws.git
...
1. DuckDns 도메인 생성
    ...
    5) Token
    ```
    the703v3.duckdns.org   
    dd9e4511-18b5-4159-8189-fed65fd12469
    ```
...
2. EC2 서버에서 DuckDns IP 자동 갱신설정
    ...
    4) 실행 권한 주기 소유자 모든권한, 그룹X, 다른 사람 X
    ```bash
    sudo chmod 700 duck.sh
    crontab -e
    2

    */5 * * * *  /home/ubuntu/duck.sh >/dev/null 2>&1
...
1) 설정파일 수정    
    ...
    server {
        listen 80;
        server_name  the703v3.duckdns.org;
        #   ... 기존내용그대로  ...
   } 
   # esc  :wq!
    ```  
...
sudo certbot --nginx -d the703v3.duckdns.org

# email 입력 - 만료알림용 , 약관동의 y, 이메일 수신 y, 리다이렉트 설정 2
```

5. 프로젝트 환경변수 및 설정 수정
> before : http://52.78.236.160
> after : https://the703v3.duckdns.org
...
http://52.78.236.160
    ...
    3) 구글 개발자
```
https://the703v3.duckdns.org/login/oauth2/code/kakao

https://the703v3.duckdns.org/login/oauth2/code/naver
