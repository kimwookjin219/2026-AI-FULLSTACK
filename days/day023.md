class A11 {
    ...
    static void classMethod() { this.a = 12; }  Q2.static은 this.a 인스턴스 사용불가

    //(5) int showZ()  인스턴스메서드
    int showZ() { 
        int a = 0;   지역변수   Q2.지역변수는 수동으로 초기화시켜줘야함
        return a; 
    }
}
...
Q5. 다음 중 오버로딩이 성립하기 위한 조건이 아닌 것은? (모두 고르시오)  c,d  메서드이름이 같고 파라미터 타입과 갯수
a. 메서드의 이름이 같아야 한다.     -오버로딩 조건 O
b. 매개변수의 개수나 타입이 달라야 한다.  -오버로딩 조건 O
