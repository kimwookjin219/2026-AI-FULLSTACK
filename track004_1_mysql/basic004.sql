...
desc userinfo;
insert into userinfo (name,age) value ('bbb',22); -- O
insert into userinfo (name,age) value (22, 'bbb'); -- X
-- 10:41:46	insert into userinfo (name,age) value (22, 'bbb')	
-- Error Code: 1366. Incorrect integer value: 'bbb' for column 'age' at row 1	0.015 sec
insert into userinfo (age, name) value (33, 'bbb');
...
SET SQL_SAFE_UPDATES=0; -- SAFE 사용 안함! 
...
insert into userinfo(no,name,age) value (1,'first',11);
insert into userinfo(no,name,age) value (2,'second',22);
insert into userinfo(no,name,age) value (3,'third',33);
insert into userinfo(no,name,age) value (4,'fourth',44);
...
update userinfo set age=44 ,name='fourth' where no=4 and name='first'; -- 조건에 안맞으면 업데이트 안됨
update userinfo set age=44 where no=4 and name='fourth';
...
create table userinfo2 select * from userinfo;  -- 테이블의 구조 + 데이터 복사해서 새로운 테이블 만들기

select * from userinfo2;

-- no 에 auto_increment , primary key 추가
alter table userinfo2 modify no int not null auto_increment primary key;
desc userinfo2; -- PK(primary key) , auto_increment 는 복제 X
