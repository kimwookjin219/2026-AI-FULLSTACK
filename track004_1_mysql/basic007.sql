...
insert into group_userinfo(no,name,age,kor,eng,math,ban,sns) values(1,'first',11,100,100,99,'A','n');
insert into group_userinfo(no,name,age,sex,kor,eng,math,ban,sns) values(2,'second',22,'m',89,92,78,'B','y');
insert into group_userinfo(no,name,age,sex,kor,eng,math,ban,sns) values(3,'third',33,'m',90,92,97,'A','y');
insert into group_userinfo(no,name,age,sex,kor,eng,math,ban,sns) values(4,'fourth',44,'f',40,42,67,'C','n');
insert into group_userinfo(no,name,age,sex,kor,eng,math,ban,sns) values(5,'fifth',55,'f',89,86,99,'B','y');
insert into group_userinfo(no,name,age,sex,kor,eng,math,ban,sns) values(6,'sixth',66,'m',10,20,44,'C','n');
...
-- 10:42:15	select ban, count(*) from group_userinfo LIMIT 0, 1000	Error Code: 1140. In aggregated query without GROUP BY, expression #1 of SELECT list contains nonaggregated column 'mbasic.group_userinfo.ban'; this is incompatible with sql_mode=only_full_group_by	0.000 sec
select ban, count(*)  -- 6줄(집계 안된 일반), 1줄 (집계)    줄 수가 달라서 오류!!
from group_userinfo;  

select ban, count(*) -- 반, 갯수
from group_userinfo
-- where 
group by ban; -- 반 별(집계 필드 뺴고 다)
-- having
-- order by
-- limit
...
-- [question12]  전체 학생 중 평균 점수가 가장 높은 반을 출력하세요.
-- +------+-----------+
-- | ban  | 평균점수  |
-- +------+-----------+
-- | A    | 96.3333   |
-- +------+-----------+ 
select ban, sum(kor+eng+math)/3 `평균점수`
from group_userinfo
group by ban,name
order by sum(kor+eng+math)/3 desc
limit 1;
...
select sum(comm) `추가수당`  -- null 빼고 집계
from  emp;      
...
-- --------------------------------------------------------
-- -- 연습문제-4
-- -- emp 테이블의 레코드 갯수를  조회하시오.  -- 집계함수는 null 빼고 계산
-- +------------+
-- | 데이터갯수 |
-- +------------+
-- |         14 |
-- +------------+
-- 1 row in set (0.00 sec)
select count(*) `데이터갯수`  -- count(*) 전체 레코드 , count(empno) 모든데이터 , count(mgr) null 빼고 계산
from emp;
...
from emp
where dept=30; -- 집계 전 필터링
-- group by dept
-- having dept=30;  -- 집계 후 필터링
...
-- +------------------------+
-- 1 row in set (0.00 sec)
select count(comm) `추가수당을 받는 사원 수`
from emp;
...
-- +---------------------------+
-- 1 row in set (0.00 sec)
select min(hiredate) `부서20 제일 오래 된 입사일`
from emp
where dept=20;
-- group by dept
-- having dept=20;
...
-- +--------------------------+
-- 1 row in set (0.00 sec) 
select avg(distinct sal) `부서30 중복제거 급여 평균`
from emp
where dept=30;
-- group by dept
-- having dept=30;
...
-- +--------+-----------+
-- 3 rows in set (0.00 sec)
select dept `deptno`, avg(sal) `평균급여`
from emp
group by dept
order by dept asc;
...
select dept `deptno`, job , avg(sal)
from emp
group by dept,job
order by dept asc;
...
select dept `deptno`, job , avg(sal) `평균급여`
from emp
group by dept,job
having avg(sal)>=2000
order by dept asc;
...
-- 집계함수의 조건은 where가 아니라 having을 써야함
...
select dept , job , count(*) `사원수`, max(sal) `최고급여`,sum(sal) `급여합`,avg(sal) `평균급여`
from emp
group by dept,job
order by dept asc;
