...
ALTER TABLE 테이블명
    ...
    DROP     삭제할 필드명
    CHANGE   이전 필드명 새로운 필드명 자료형 옵션
    MODIFY   수정할 필드명 자료형 옵션
    RENAME   새로운 테이블 이름
...
#3) 필드 수정(CHANGE) - 필드명 수정
    alter table userinfo change oldname new name 자료형 옵션
...
#5) 테이블명 수정(RENAME) (add, drop, change | modify , rename)
...
alter table alter_coffee drop primary key; (primary key 빼기)
