...
public class Repeat002_oop {
	public static void main(String[] args) {
		// Q3. Papa p1 = new Papa(); 실행 시 메모리 구조와 출력 결과를 설명하시오.
		       Papa p1 {money=10/toString} = 1번지 {money=10/toString}
		...
		// Q5.Son s3 = (Son) new Papa(); 실행 시 어떤 문제가 발생하는가?  s3이 초기화를 해서 객체를 만든 적이 없어서 불가능
		...
		// Q7. p4에서 Son의 money를 사용하는 방법은? 타입캐스팅
...
오버로딩 : 메서드 이름은 같고 알규먼트 자료형과 갯수를 다르게 해서 구분(비슷한 기능)
오버라이딩 : 상속 시 부모클래스의 메서드를 자식 클래스에 맞게 수정 후 사용
