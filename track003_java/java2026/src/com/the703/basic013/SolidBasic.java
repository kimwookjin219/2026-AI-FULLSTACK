...
//2. O : 개방 폐쇄 원칙(새로운 쿠키 추가가능, 기존코드 건들지말기)
...
//3. L : 리스코프 치환 ( 어떤 쿠키든 Cookie 인터페이스로 바꿔도 문제 없음 / 부모 = 자식 )
...
//5. D : 의존역전 - 어떤 쿠키든 가게에서 팔 수 있음
//               CookieFactory는 구체적인 쿠키가 아니라 Cookie에 의존
class CookieShop{
	private Cookie cookie; // interface Cookie 
	public CookieShop() { super();  }
	public CookieShop(Cookie cookie) { super(); this.cookie = cookie; } // 쿠키 종류는 외부에서 넣어줌
...
public class SolidBasic {
	public static void main(String[] args) {
		//1. S : 단일 책임 원칙
		System.out.println("1. S : 단일 책임 원칙");
		CookieMaker maker = new CookieMaker();
		maker.bakeCookie("초코"); maker.bakeCookie("오트밀"); maker.bakeCookie("라즈베리");
		
		//2. O : 개방-폐쇄의 원칙 + L : 리스코프 치환
		System.out.println("2. O : 개방-폐쇄의 원칙(레시피 추가) + L : 리스코프(공장- 어떤 쿠키든지 굽기 가능) 치환");
...
S : 한 클래스는 하나만(한가지 일만 / 쿠키 반죽 - 반죽만 , 쿠키 굽기 - 쿠키굽기만 ) 
O : (수정말고 추가만) 새로운 쿠키를 쉽게 추가
