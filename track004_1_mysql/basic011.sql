...
select e.deptno,d.dname,avg(sal) `avg_sal`, max(sal) `max_sal`, min(sal) `min_sal`, count(*) `cnt`
from emp e , dept d
...
-- Q1) any 1개 < 최대보다 작다 -> 5보다 작다
select num from atest where num in (3,4,5) ; -- 3,4,5

select num 
from atest 
where num < any( select num from atest where num in (3,4,5) )  -- 5보다 작다
order by num;     

-- Q2)
select num 
from atest 
where num > any( select num from atest where num in (3,4,5) ) -- 3보다 크다
order by num;  

-- Q3) 
select   num 
from     atest  
where    num < all(    select num  from atest  where num in(3,4,5)    )  -- 3보다 작다
order by num;
...
select   num 
from     atest  
where    num > all(    select num  from atest  where num in(3,4,5)    )  -- 5보다 크다
order by num;               
...
-- in (subquery) - 서브쿼리 결과 집합 중 하나와 같으면 true (값이 집합 안에 있는지 확인)
...
select *
from emp
where (deptno,sal) = some(select deptno,max(sal) from emp group by deptno );
...
-- ANY를 이용하여 30번 부서의 최고 급여보다 작은 급여를 받는 사원정보
...
select *
from emp
where exists (select dname from emp where deptno=50);
...
-- +-------+--------+--------+------------+----------+
-- 3 rows in set (0.00 sec)
select e.empno,e.ename,d.deptno,dname,loc
from (select empno,deptno,ename from emp where deptno=10) e join (select deptno,dname,loc from dept where deptno=10) d;
