...
- pakage.json
{
  ...
  "scripts": {
    ...
    "start": "cross-env NODE_ENV=production next start -p 3060", //배포(production) - 3060 포트 실행
  ...
  "dependencies": {
    ...
    "@next/bundle-analyzer": "^12.3.4", // 사용하는 용량 - 시각분석 도구
    "@reduxjs/toolkit": "^1.9.7",  // 전역 저장공간
    "antd": "^4.8.6",
    "axios": "^1.3.4",  // 백엔드 api 서버와 통신 http 라이브러리
    "babel-plugin-styled-components": "^1.12.0", // ssr 환경
    "bootstrap": "^5.3.8", // 전반적인 웹 스타일링
    "cross-env": "^7.0.2", // 운영체제 상관없이 (window, mac, linux) 환경변수 설정
    "dotenv": "^16.0.1", // 중요 키
    "express": "^5.2.1", // 웹서버
    "faker": "^4.1.0",  // 더미데이터(가짜 데이터) 무작위 생성도구
    "immer": "^9.0.19", // 코드 짧게 줄이기
    "jest": "^29.7.0", // 테스트 도구
    "moment": "^2.29.1", // 날짜 및 시간 데이터 포맷팅
    "next": "^12.3.4", // react에서 ssr 지원 프레임워크
    "next-redux-wrapper": "^6.0.2",  // 연결
    "pm2": "^4.5.0", // 배포환경에서 node.js 프로세스 계속 실행되게 관리
    "prop-types": "^15.7.2", // 전달되는 파라미터(props) 타입 검사
    "react": "^17.0.2", 
    "react-dom": "^17.0.2",  // 브라우저 dom 렌더링 도구
    "react-redux": "^8.0.5", 
...
//2. react 테스트가 끝나면 cleanup 자동실행 - dom 정리
...
1) [components] - Layout.js   공통 틀 (헤더,네비게이션, 푸터)
2) [pages] - _app.js          모든 페이지들을 `Layout`으로 감싸는 진입점
3) [pages] - index.js         본문 컨텐츠

> _app.js Layout 감싸서 영역 안에 {children}
step1) localhost:300. 접속
...
front/
├── pages/                 
│   ├── _app.js  # Next.js 기반 페이지 설정 - 전체 앱의 ㅡ공통설정
│   ├── index.js # 메인 페이지
│   ├── login.js # 로그인 페이지
│   ├── signup.js # 회원가입 페이지
│   ├── users.js # 사용자 목록, 정보 페이지
```
...
2. `useEffect`   → 로그인 여부 확인 및 사용자 목록 불러오기
  - 생명주기 및 부수 효과 처리
  - 부품 맨 처음 나올때(mount), 사라질때, 특정상태 변경 시 
