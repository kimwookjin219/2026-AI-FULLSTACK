...
print("\n\n--- [1] Python 핵심 ---")
print("\n--- 0. 변수 선언 및 출력 ---")
### int, String / const,  let, var 키워드 필요X 
...
fruits.append("orange") # js push() , java add() , python append()
print(f"orange추가 과일리스트 : {fruits}")    
print(f"index의 0번째 과일 : {fruits[0]}")
print(f"index의 -1번째 과일 : {fruits[-1]}")  # -1은 맨뒤의 요소 추출

print("\n--- 4. 딕셔너리(Dictionary) ---")
# java `HashMap` Key - Value 구조
# json 형태로 데이터 가공
user_info = {"name" : "홍길동" , "role" : "admin" , "level" : 5} 
user_info["level"] += 1 # 기존의 Key의 Value 수정 / 없으면 key-value 값 추가
...
# for members , 처리 : "OFFLINE"을 Value로 갖는 결과: 딕셔너리
...
# 파이썬의 list 메모리 주소를 참조해서 속도가 느림

print("\n\n--- [2] NumPy 기초 ---") # a ze a
import numpy as np
# np.array([])           : 기존 파이썬 리스트를 NumPy 배열로 변환
# np.zeros(shape)        : 지정 크기만큼 0으로 채워진 배열
# np.arange(start, end) : start 부터 end 전까지 연속된 배열 

print("\n--- 1. 1차원 및 2차원 배열(NDArray) 생성 ---")
# 1. 배열
# shape : 1차원배열(5,)  2차원배열(2,3)  3차원(2,2,3) 높이 2, 각 층 2행3열
...
print(f"arange(1,6) 1d 배열 : {seq_arr}")
...
#     [500, 60000]
# ])
# 매출액이 30000넘었어? [False, True, True]
# sql : where sales >= 30000
# [행,열] 이용해서 매출액만 뽑아오기
sales_30000 = data_2d[:,1] >= 30000
print(f"매출액 30000이상 필터링 : {sales_30000}")   # [False  True  True]
print(f"필터링된 실제 데이터 : {data_2d[sales_30000]}") # [[  300 50000] [  500 60000]]


print("\n--- 4. 수치 통계 연산 및 Axis(축) 연산 ---")
## 통계 함수 : 평균-mean , 합계-sum , 최대값-max , 표준편차-std , 
print("방문자 수 평균 : ",np.mean(visitors))
print("총 매출액 합계 : ",np.sum(sales))
print("방문자 수 최대 : ",np.max(visitors))
print("매출액 표준편차 : ",np.std(sales))  # 평균에서 멀어진 정도
# 표준편차: 100,100,100 (평균 100) 편차 0 
#           0,100,200 (평균 100)-> 날마다 기복이 크다 (표준편차가 크다)

# axis(축) 옵션
# axis=None (기본값) : 전체 요소를 통틀어서 계산
# axis=0 : 열(Column) 방향으로 계산 (세로 방향으로)
# axis=1 : 행 방향으로 계산 (가로 방향으로)
col_sums = np.sum(data_2d , axis=0) # 결과 확인
print("Axis=0 열 방향 계산 : ", col_sums)
...
# ==============================================================================
# Pandas - 엑셀이나 관계형 데이터베이스(RDB)의 테이블 형태
# 핵심 객체
...
#     {'date': '2026-06-03', 'category': '커머스', 'visitor_count': 500, 'sales_amount': 80000},
# ]
# df[ 컬럼 > 조건 ] 특정 조건의 행만 추출 (가로 방향의 데이터)
# df[새컬럼 명] = 연산식  기존 컬럼 계산 새로운 컬럼 생성

# 방문자 수가 200명 이상인 행
over_200_df= df[ df['visitor_count'] >= 200 ]
df['avg_spend'] = df['sales_amount'] / df['visitor_count']  # 1인당 평균결재 금액

print(f"방문자 200명 이상 필터링 : {over_200_df}")
print(f"총 매출액 : {df['sales_amount']}")
print(f"1인당 평균 결재금액 : {df}")
...
# tolist() : 파이썬 List `[]` 포맷변환  Chart.js, 그래프 그릴때
