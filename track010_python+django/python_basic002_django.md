...
```bash
#1. 프로젝트별 독립된 파이썬 라이브러리 실행 환경구축 (virtual environment)
# 작업 폴더 이동 후 가상환경 생성 
python -m  venv venv
...
```bash
# 타 언어의 MVC 패턴과 거의 동일 (mvc=mtv)
# JAVA
# Model    - 데이터 관련  : Entity/Dao/Dto
# Template - 화면 구성    : html, jsp, Thymeleaf ...
# View     - 비즈니스 로직 : Controller ...
```
...
```bash
# 장고 프로젝트는 여러 개의 App(기능단위 모듈)로 구성
# analytics 앱 생성
...
│   ├── apps.py
│   ├── models.py      <-- [DB] ServiceLog 데이터 모델 정의 (jpa Entity 역할)
...
class ServiceLog(models.Model):
    ...
    # DateField : YYYY-MM-DD 날짜 필드
    date=medels.DateField(verbose_name="날짜")
    category=models.CharField(max_length=50, verbose_name="카테고리")
    visitor_count=models.IntegerField(default=0, verbose_nane="방문자 수")
    sales_amount=models.IntergerField(default=0, verbose_name="매출액")
    # Java toString 해당기능
    def __str__(self):
...
# 2. DB 적용 - DB.sqlite3 데이터베이스에 반영해서 테이블을 생성
...
# 답: 1) python manage.py ( makemigration ) / 2) python manage.py ( migrate )
...
## M(model) T(html,css: 화면) V(Controller: 비즈니스 로직, DB와 view와 연결)
...
urlpatterns= [
    # http://127.0.0.1:8000/dashboard/ 요청시 views.py의 dashboard_view 연결
